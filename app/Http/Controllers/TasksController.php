...
class TasksController extends Controller
{
    ...
    {   
        ...
        //      collect(['external_id', 'title', 'created_at', 'deadline', 'user_assigned_id', 'status_id', 'client_id','description'])
        //          ->map(function ($field) {
        //              return (new Task())->qualifyColumn($field);
        //         })
        //         ->all()
        // );
        $tasks = Task::join('users as u','u.id','=','tasks.user_assigned_id')
        ->join('statuses as s','s.id','=','tasks.status_id')
        ->join('clients as c','c.id','=','tasks.client_id')
        ->select(['tasks.external_id as external_id','tasks.title','tasks.created_at','tasks.deadline','s.title as status','c.company_name as client','tasks.description','u.name as user']);
        //dd($tasks);

        if ($user = $request->get('user_id')) {
            $tasks->where('tasks.user_assigned_id', $user);
        }

        if ($request->get('date_from') != null && $request->get('date_to') != null) {
            $from = $request->get('date_from');
            $to = $request->get('date_to');
            $tasks->whereBetween('sample_data.created_at', [$from,$to]);
        }
        return Datatables::of($tasks)
            ...
            ->editColumn('deadline', function ($tasks) {
                return Carbon::parse($tasks->deadline)->format('y/m/d');
            })
